...
screen_settings = {
    "FULLSCREEN": (info_object.current_w, info_object.current_h),
    "WINDOWED": (info_object.current_w / 2, info_object.current_h / 2),
    "CURRENT": (info_object.current_w, info_object.current_h),
}

screen = pygame.display.set_mode(screen_settings["FULLSCREEN"], FULLSCREEN)

display = pygame.Surface((1920, 1080))

def scalePos(tup):
    x, y = tup
    rx = 1920 / screen_settings["CURRENT"][0]
    ry = 1080 / screen_settings["CURRENT"][1]
    return x * rx, y * ry

def toggleFullscreen():
    global screen
    if screen_settings["CURRENT"][0] == screen_settings["WINDOWED"][0]:
        screen = pygame.display.set_mode(screen_settings["FULLSCREEN"], pygame.FULLSCREEN)
        screen_settings["CURRENT"] = screen_settings["FULLSCREEN"]
        draw()
    else:
        screen = pygame.display.set_mode(screen_settings["WINDOWED"])
        screen_settings["CURRENT"] = screen_settings["WINDOWED"]
        draw()
...
overlay_img = pygame.transform.scale(pygame.image.load('data/images/black_overlay.png').convert(), (1920, 1080))
...
class Player():
    ...
    def update(self):
        self.move()
        self.looking(scalePos(pygame.mouse.get_pos()))
...
class Button():
    ...
    def is_over(self):
        mx, my = scalePos(pygame.mouse.get_pos())
...
def draw_main_menu():
    ...
    screen.blit(pygame.transform.scale(display, screen_settings["CURRENT"]), (0, 0))
...
def draw():
    ...
    gun.draw(gun.get_angle(scalePos(pygame.mouse.get_pos())))
    ...
    if fade_out:
        fade_alpha += 7
        fade_surface = pygame.Surface(screen_settings["CURRENT"])
...
while True:
    ...
    if game_running:
        ...
        scroll[0] += int((player.rect.x - scroll[0] - (screen_settings["CURRENT"][0] / 2 + player.width / 2)) / 18) * dt
        scroll[1] += int((player.rect.y - scroll[1] - (screen_settings["CURRENT"][1] / 2 + player.height / 2)) / 18) * dt
