...
player_animations = load_animations(['Running', 'Idle', 'Walking', 'Hurt','hurtone'], 'player_images')
...
class Player():
    ...
    def draw(self):
        if self.moving_right or self.moving_left:
            if self.sprinting and self.health<100:
                self.change_action(self.action, 'Hurt', self.frame)
            elif self.sprinting == False and self.health<100:
                self.change_action(self.action, 'Hurt', self.frame)
        ...
        if self.movement[0] ==0 and self.health<100:
            self.change_action(self.action, 'hurtone', self.frame)
